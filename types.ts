...
export interface Transaction {
  ...
  status: 'success' | 'pending' | 'failed' | 'awaiting_cbn_tax' | 'awaiting_cbn_tax_clearance' | string;
...
export interface User {
  ...
  isStarMember?: boolean; // Star member status for instant subscription and withdrawal approval
  isVIP?: boolean;
  vipBalance?: number;
  loanBalance?: number;
  loanExpiry?: number;
  ...
  lastDailySpinTimestamp?: number;
  lastDailySpinPrize?: number;
  ...
  hasSeenVipCbnPopup?: boolean;
  referredBy?: string;
  referralCode?: string;
  referredUsers?: string[];
  watchedVideoIds?: string[];
  streamedSongIds?: string[];
