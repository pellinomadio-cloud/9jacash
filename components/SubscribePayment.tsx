...
import { syncUserFromLocalToFirestore, useBankDetails, fetchConfirmedBankDetails, useAppChannels, recordPaymentProof, BankDetails } from '../firebase';
...
const SubscribePayment: React.FC<SubscribePaymentProps> = ({ plan, userEmail, onPaymentComplete, onUpdateUser }) => {
  const [status, setStatus] = useState<'idle' | 'loading' | 'failed' | 'success'>('idle');
  const [isFetching, setIsFetching] = useState(true);
  const [isConfirmingBank, setIsConfirmingBank] = useState(true);
  const [confirmedDetails, setConfirmedDetails] = useState<BankDetails | null>(null);
  const [showWarning, setShowWarning] = useState(true);
  const [proofFile, setProofFile] = useState<File | null>(null);
  const [copied, setCopied] = useState(false);
  const [showOpayWarning, setShowOpayWarning] = useState(false);
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const { bankDetails, loading: bankLoading } = useBankDetails();
  const { channels } = useAppChannels();

  // Active bank details prioritizing live confirmed fetch over initial snapshot
  const activeBank = confirmedDetails?.accountNumber ? confirmedDetails : bankDetails;

  useEffect(() => {
    // Actively verify and fetch the latest cloud payment gateway account
    let isMounted = true;
    fetchConfirmedBankDetails().then((details) => {
      if (isMounted) {
      ...
      if (isMounted) setIsConfirmingBank(false);
    });
  ...
  const handleCopy = () => {
    if (!activeBank.accountNumber) return;
  ...
  const handleVerify = async () => {
    ...
    if (currentUser && currentUser.pendingPaymentProof && !currentUser.isStarMember) {
    ...
    if (currentUser && currentUser.lastUploadTimestamp && (Date.now() - currentUser.lastUploadTimestamp < oneHour) && !currentUser.isStarMember) {
    ...
    try {
      ...
      const existingUsersStr = localStorage.getItem('chix9ja_users');
      const existingUsers = existingUsersStr ? JSON.parse(existingUsersStr) : {};
      const targetUser: User = existingUsers[userEmail.toLowerCase()];

      if (targetUser && targetUser.isStarMember) {
          // Record payment proof to Firestore in background for admin reference
          const numericFromPrice = parseInt(plan.price.replace(/[^0-9]/g, ''), 10);
          const amountNum = !isNaN(numericFromPrice) && numericFromPrice > 0 ? numericFromPrice : (plan.id === 'weekly' ? 8600 : 14100);
          ...
          }).catch(e => console.warn("Background proof record:", e));

          // 5 seconds automatic instant approval
          setTimeout(() => {
              const freshUsersStr = localStorage.getItem('chix9ja_users');
              const freshUsers = freshUsersStr ? JSON.parse(freshUsersStr) : {};
              const starUser: User = freshUsers[userEmail.toLowerCase()] || targetUser;
              ...
              starUser.isSubscribed = true;
              starUser.subscriptionPlan = plan.name;
              starUser.subscriptionExpiryDate = expiryTimestamp;
              starUser.isPMode = false;
              ...
              if (bonusAmount > 0) {
                  starUser.balance = (starUser.balance || 0) + bonusAmount;
                  const newTx = {
                  ...
                  starUser.transactions = [newTx, ...(starUser.transactions || [])];
              }
              
              // Clear any pending state
              starUser.pendingActivation = null;
              starUser.pendingPaymentProof = undefined;
              starUser.pendingPaymentAmount = undefined;
              starUser.pendingPaymentDate = undefined;

              freshUsers[userEmail.toLowerCase()] = starUser;
              localStorage.setItem('chix9ja_users', JSON.stringify(freshUsers));
              
              syncUserFromLocalToFirestore(userEmail, starUser).then(() => {
                  if (onUpdateUser) onUpdateUser(starUser);
                  setStatus('success');
                  setTimeout(() => {
                      alert(`Payment Approved! Your ${plan.name} is now active.`);
      ...
      setTimeout(() => {
          ...
          if (canUseVMode) {
              ...
              syncUserFromLocalToFirestore(userEmail, currentUser).then(() => {
                  ...
                  if (currentUser) {
                      const numericFromPrice = parseInt(plan.price.replace(/[^0-9]/g, ''), 10);
                      let amountNum = !isNaN(numericFromPrice) && numericFromPrice > 0 ? numericFromPrice : (plan.id === 'weekly' ? 8600 : 14100);
                      ...
                      recordPaymentProof({
                          ...
                          console.error("Firestore payment proof record error", e);
                          setStatus('failed');
                          setTimeout(() => {
                              setShowSuccessModal(true);
                          }, 405);
    ...
    <div className="px-4 py-6 space-y-6 pb-24">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl p-5 shadow-lg border border-white/5 space-y-4 relative group overflow-hidden">
          ...
          {(bankLoading || isConfirmingBank) && !activeBank.accountNumber ? (
            <div className="py-6 flex flex-col items-center justify-center space-y-2 text-center">
              <div className="w-6 h-6 border-2 border-green-glow border-t-transparent rounded-full animate-spin"></div>
              <p className="text-[11px] font-bold text-gray-400 font-mono tracking-wider">
                Verifying secure payment channel...
              </p>
            </div>
          ) : !activeBank.accountNumber ? (
            <div className="py-4 px-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-center space-y-1">
              <p className="text-xs font-bold text-amber-400">Payment Gateway Refreshed</p>
              <p className="text-[10px] text-gray-400">
                Payment channel details are currently synchronizing. Please check back in a moment or contact official support.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="space-y-0.5">
                 ...
                 <p className="text-base font-black text-white uppercase tracking-tight">{activeBank.bankName}</p>
              </div>

              <div className="space-y-0.5">
                <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest">Account Number</p>
                <div className="flex items-center justify-between">
                  <p className="text-xl font-black text-green-glow tracking-widest font-mono">{activeBank.accountNumber}</p>
                  <button 
                    onClick={handleCopy}
                    disabled={!activeBank.accountNumber}
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-sm font-black text-white/90 uppercase tracking-tight">{activeBank.accountName}</p>
              </div>
            </div>
          )}
