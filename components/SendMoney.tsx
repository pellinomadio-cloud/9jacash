...
const SendMoney: React.FC<SendMoneyProps> = ({ user, onTransfer, onSubscribeRedirect, onGoToUpgrade, onGoHome, onRequestFreeWithdrawal, onViewHistory }) => {
  ...
  // Guard: unsubscribed members cannot stay on SendMoney
  useEffect(() => {
    if (!user.isSubscribed) {
      onGoHome(true);
    }
  }, [user.isSubscribed, onGoHome]);
  ...
  const checkWithdrawalLimit = (transferAmount: number): string | null => {
    if (user.isStarMember) return null; // Star members have unrestricted instant withdrawals
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!accountName || accountName.trim().length === 0) {
        setError("Please enter a valid account name (select bank and enter 10-digit account number).");
        return;
    }
    ...
    if (transferAmount > user.balance) {
        setError("Insufficient funds in your account balance.");
        return;
    }
    ...
    try {
        ...
        // Execute transfer state update without submitting to admin dashboard
    ...
    return (
      <div className="min-h-screen bg-black text-white font-sans p-4 sm:p-6 pb-24">
        <div className="max-w-md mx-auto space-y-5 animate-in fade-in duration-300">
          <div className="bg-zinc-900 border border-red-500/40 rounded-3xl p-6 shadow-xl text-center space-y-5">
            ...
            <div>
              <h2 className="text-xl font-black text-white uppercase tracking-tight mb-2">Withdrawal Restricted</h2>
              <div className="bg-red-500/10 p-4 rounded-2xl border border-red-500/20">
                <p className="text-red-300 font-bold text-xs leading-relaxed">
  ...
  if (step === 'success') {
    const isStar = Boolean(user.isStarMember);
    const parsedAmount = parseFloat(amount || '0');

    return (
      <div className="min-h-screen bg-black text-white font-sans p-4 sm:p-6 pb-24">
        <div className="max-w-md mx-auto space-y-5 animate-in fade-in duration-300">
          <div className="bg-zinc-900 border border-emerald-500/40 rounded-3xl p-6 shadow-xl text-center space-y-5 relative overflow-hidden">
            ...
            {isStar ? (
              <div>
                ...
                <p className="text-xs text-zinc-300 font-medium leading-relaxed">
                  Your withdrawal of <span className="font-extrabold text-emerald-400 font-mono">₦{parsedAmount.toLocaleString()}</span> to <strong className="text-white">{accountName}</strong> was completed successfully.
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-xs text-zinc-300 font-medium leading-relaxed">
                  You successfully submitted a withdrawal of <span className="font-extrabold text-amber-400 font-mono">₦{parsedAmount.toLocaleString()}</span> to {accountName}.
                </p>
              </div>
            )}

            {!isStar && (
              <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl text-left flex items-start space-x-3 text-amber-200">
                <Star size={24} className="shrink-0 text-amber-400 mt-0.5 fill-amber-500" />
                <div>
                  <h3 className="font-black text-xs uppercase tracking-wider text-amber-300">Withdrawal Processed</h3>
                  <p className="text-[11px] mt-1 font-semibold leading-relaxed text-zinc-300">
                    Your withdrawal request of <strong className="text-amber-400">₦{parsedAmount.toLocaleString()}</strong> to {bank} has been placed.
                  </p>
                </div>
              </div>
            )}
            ...
            {/* Details of Transfer */}
            <div className="bg-zinc-950/80 p-4 rounded-2xl border border-zinc-800 space-y-2 text-xs text-left font-sans">
              <div className="flex justify-between items-center py-1.5 border-b border-zinc-850">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Transfer Status</span>
                <span className={`font-black uppercase text-xs flex items-center gap-1.5 ${isStar ? 'text-emerald-400' : 'text-amber-400'}`}>
                  <span className={`w-2 h-2 rounded-full ${isStar ? 'bg-emerald-400' : 'bg-amber-400'}`}></span>
                  {isStar ? 'SUCCESSFUL' : 'PROCESSING'}
                </span>
              </div>
              <div className="flex justify-between items-center py-1.5 border-b border-zinc-850">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Amount</span>
                <span className={`font-mono font-black text-sm ${isStar ? 'text-emerald-400' : 'text-amber-400'}`}>
                  ₦{parsedAmount.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center py-1.5 border-b border-zinc-850">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Destination Bank</span>
                <span className="font-extrabold text-white">{bank}</span>
              </div>
              <div className="flex justify-between items-center py-1.5 border-b border-zinc-850">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Account Number</span>
                <span className="font-mono font-bold text-white tracking-wider">{accountNumber}</span>
              </div>
              <div className="flex justify-between items-center py-1.5 border-b border-zinc-850">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Account Name</span>
                <span className="font-extrabold text-white">{accountName}</span>
              </div>
              <div className="flex justify-between items-center py-1.5">
                <span className="text-zinc-400 font-bold uppercase text-[10px] tracking-wider">Transaction Reference</span>
                <span className="font-mono font-bold text-zinc-400 text-[11px]">CHX-TRX-{Date.now().toString().slice(-8)}</span>
              </div>
            </div>

            {/* Actions: NO VIP Upgrade for Star Members! */}
            <div className="space-y-2">
              {!isStar && onGoToUpgrade && (
                <button 
                  type="button"
                  onClick={onGoToUpgrade}
                  className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-black rounded-2xl shadow-lg shadow-blue-500/30 transition-all active:scale-95 uppercase tracking-wider text-xs flex items-center justify-center space-x-2 cursor-pointer"
                >
                  <Icons.Upgrade size={16} />
                  <span>Upgrade to VIP (Remove Pending)</span>
                </button>
              )}
              {onViewHistory && (
                <button 
                  ...
                  className={`w-full py-3.5 ${
                    isStar 
                      ? 'bg-gradient-to-r from-emerald-500 via-emerald-400 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-black shadow-emerald-500/30' 
                      : 'bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-500 hover:from-amber-500 hover:to-yellow-600 text-black shadow-amber-300/50'
                  } font-extrabold rounded-2xl shadow-lg transition-all active:scale-95 uppercase tracking-wider text-xs cursor-pointer flex items-center justify-center gap-2`}
                >
                  <Receipt size={16} />
                  <span>{isStar ? 'View Receipt in Transaction History' : 'View Transaction History'}</span>
                </button>
              )}
              <button 
                type="button"
                onClick={() => onGoHome(false)}
                className="w-full py-3 bg-zinc-800 border border-zinc-700 text-zinc-300 font-bold rounded-2xl hover:bg-zinc-700 transition-all active:scale-95 uppercase tracking-wider text-xs cursor-pointer"
              >
                Done
              </button>
            </div>
      ...
      <div className="max-w-md mx-auto space-y-5 animate-in fade-in duration-300">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 shadow-xl space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-black uppercase tracking-wider text-zinc-300 flex items-center gap-1.5 mb-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full p-3.5 bg-zinc-950 border border-zinc-800 rounded-2xl text-white placeholder-zinc-500 outline-none focus:border-amber-500 focus:bg-zinc-900 font-medium text-sm transition-all"
                />
                <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                  {bank ? (
                    <span className="text-[10px] font-extrabold text-black bg-amber-400 px-2.5 py-1 rounded-full border border-amber-300 uppercase tracking-wider">
                      Selected
                    </span>
                  ) : (
                    <Icons.ChevronRight className="text-zinc-500 rotate-90" size={18} />
                  )}
                </div>

                {isBankDropdownOpen && (
                  <div className="absolute z-50 w-full mt-1.5 max-h-60 overflow-y-auto bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl divide-y divide-zinc-800 animate-in fade-in slide-in-from-top-1 duration-100">
                    {(() => {
                      ...
                      return filtered.length > 0 ? (
                        filtered.slice(0, 15).map((b) => (
                          <button
                            ...
                            className="w-full text-left px-4 py-3 text-sm text-zinc-200 hover:bg-zinc-800 font-medium transition-colors flex items-center justify-between"
                          >
                        ...
                        <div className="px-4 py-3 text-sm text-zinc-400 italic text-center">
                          No banks found
                        </div>
                      );
                    })()}
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full p-3.5 bg-zinc-950 border border-zinc-800 rounded-2xl text-white placeholder-zinc-500 outline-none focus:border-amber-500 focus:bg-zinc-900 font-mono text-lg font-bold tracking-wider transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className={`w-full p-3.5 bg-zinc-950 border rounded-2xl font-bold outline-none transition-all ${
                    isVerifying 
              ...
              {verificationError && (
                <p className="text-xs text-red-400 font-bold mt-1.5 pl-1 flex items-center gap-1">
            ...
            <div>
              <label className="text-xs font-black uppercase tracking-wider text-zinc-300 flex items-center justify-between gap-1.5 mb-1.5">
              ...
              <div className="relative">
                <span className="absolute left-3.5 top-3.5 text-amber-400 font-black">₦</span>
                <input
                  type="number"
                  value={amount}
                  ...
                  required
                  min="1"
                  className="w-full p-3.5 pl-10 bg-zinc-950 border border-zinc-800 rounded-2xl text-xl font-black text-white placeholder-zinc-500 outline-none focus:border-amber-500 focus:bg-zinc-900 transition-all"
                />
              </div>
              <div className="flex justify-between items-center mt-1.5">
                {user.isSubscribed && (
                  <div className="flex items-center text-black bg-amber-400 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter border border-amber-300 shadow-sm animate-bounce">
                    <Star size={10} className="mr-1 fill-black" /> Golden Member
                  </div>
                )}
                <p className="text-xs font-bold text-zinc-400 text-right flex-1">
        ...
        {showSubscribePopup && (
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-zinc-900 border border-amber-500/40 rounded-3xl p-6 w-full max-w-sm text-center space-y-5 shadow-2xl relative overflow-hidden">
              <div className="w-16 h-16 bg-amber-500/10 border border-amber-500/30 rounded-full flex items-center justify-center mx-auto text-amber-400 shadow-md animate-bounce">
              ...
              <div className="pt-2 space-y-2">
                ...
                <button
                  ...
                  className="w-full py-3 bg-zinc-800 border border-zinc-700 text-zinc-300 font-bold rounded-2xl hover:bg-zinc-700 transition-all text-xs uppercase"
                >
                  Done
                </button>
              </div>
