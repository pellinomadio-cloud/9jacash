...
import { db, syncUserFromLocalToFirestore, doc, setDoc } from '../firebase';
  ...
  useEffect(() => {
    ...
    const verifyTransaction = async () => {
      try {
        ...
        if (paymentType === 'subscription_weekly') expectedAmount = 8600;
        else if (paymentType === 'subscription_quarterly') expectedAmount = 48000;
        else if (paymentType === 'subscription_monthly') expectedAmount = 14100;
        ...
        if (paymentType.startsWith('subscription_')) {
          ...
          finalUser.pendingActivation = null;
          finalUser.showVipWithdrawalNotice = false;
          finalUser.persistentVipNotice = false;
          finalUser.hasSeenVipCbnPopup = true;
          if (finalUser.transactions) {
            finalUser.transactions = finalUser.transactions.map(t => {
