...
const Subscribe: React.FC<SubscribeProps> = ({ onPlanSelect, userBalance }) => {
  ...
  const plans: PlanDisplay[] = basePlans.map(p => {
    const scaledNumeric = priceMultiplier > 1 
      ? Math.round((p.basePrice * priceMultiplier) / 500) * 500
      : p.basePrice;
    return {
      ...p,
    ...
    <div className="px-4 py-8 space-y-10 relative pb-32 overflow-hidden">
      ...
      <div className="space-y-3">
        {visiblePlans.map((plan, index) => {
          // No restriction forcing new users to monthly when balance passes 200,000
