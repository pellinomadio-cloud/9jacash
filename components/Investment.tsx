...
const Investment: React.FC<InvestmentProps> = ({ user, onBack, onUpdateUser, onGoToUpgrade }) => {
  ...
  // Unsubscribed user: show "Coming Soon"
  if (!user.isSubscribed) {
    return (
      <div className="px-4 py-16 flex flex-col items-center justify-center space-y-6 text-center animate-in fade-in duration-500 max-w-sm mx-auto">
        <div className="relative">
          <div className="w-20 h-20 bg-gradient-to-br from-amber-500/20 to-yellow-500/10 rounded-3xl border border-amber-500/30 flex items-center justify-center shadow-[0_0_30px_rgba(245,158,11,0.2)]">
            <Icons.Clock size={38} className="text-amber-400" />
          </div>
          <span className="absolute -bottom-2.5 left-1/2 -translate-x-1/2 px-2.5 py-0.5 bg-amber-500 text-black text-[9px] font-black uppercase tracking-wider rounded-full shadow-md whitespace-nowrap">
            COMING SOON
          </span>
        </div>
        ...
        <div className="space-y-2 pt-2">
          ...
          <p className="text-xs text-zinc-400 max-w-[260px] mx-auto leading-relaxed">
            The Chix9ja Investment Desk is currently undergoing system upgrades and will be launched soon. Stay tuned!
          </p>
        </div>

        <button 
          onClick={onBack}
          className="w-full py-3 bg-zinc-900 hover:bg-zinc-800 text-white font-black rounded-xl uppercase tracking-wider text-xs border border-zinc-800 shadow-lg active:scale-95 transition-all cursor-pointer"
        >
          BACK TO DASHBOARD
        </button>
      </div>
  ...
  // Subscribed user but not VIP: show "VIP Required"
  if (!user.isVIP) {
    return (
      <div className="px-4 py-16 flex flex-col items-center justify-center space-y-6 text-center animate-in fade-in duration-500 max-w-sm mx-auto">
        <div className="w-20 h-20 bg-amber-500/10 rounded-3xl border border-amber-500/30 flex items-center justify-center shadow-[0_0_30px_rgba(245,158,11,0.15)]">
          <Icons.Lock size={38} className="text-amber-400" />
        </div>
        <div className="space-y-2">
          <span className="px-3 py-1 bg-amber-500/15 border border-amber-500/40 rounded-full text-amber-300 text-[10px] font-mono font-black uppercase tracking-wider inline-block">
            VIP EXCLUSIVE
          </span>
          <h2 className="text-2xl font-black text-white uppercase tracking-tight">VIP Required</h2>
          <p className="text-xs text-zinc-400 max-w-[260px] mx-auto leading-relaxed">
            VIP status is required before opening the investment desk. Please upgrade to VIP to access investment plans.
          </p>
        </div>
        <div className="space-y-2.5 w-full pt-1">
          {onGoToUpgrade && (
            <button 
              ...
              className="w-full py-3.5 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:from-amber-300 hover:to-yellow-300 text-black font-black rounded-xl uppercase tracking-wider text-xs shadow-[0_0_20px_rgba(245,158,11,0.35)] active:scale-95 transition-all cursor-pointer"
            >
              UPGRADE TO VIP
            </button>
          )}
          <button 
            onClick={onBack}
            className="w-full py-3 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 hover:text-white font-bold rounded-xl uppercase tracking-wider text-xs border border-zinc-800 shadow-md active:scale-95 transition-all cursor-pointer"
          >
            BACK TO DASHBOARD
          </button>
        </div>
